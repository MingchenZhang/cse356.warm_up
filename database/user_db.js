            ...
            userDB.userBasicColl.updateMany({username: username}, {$set: userDoc}, {
                ...
                if (!err) {
                    if(result.upsertedId && result.upsertedId._id){
                        if(memcached) memcached.set(MEMCACHED_USERID+result.upsertedId._id, JSON.stringify(userDoc), 3600, (err)=>{if(err) console.error(err)});
                    }
                    if(memcached) memcached.set(MEMCACHED_USERNAME+username, JSON.stringify(userDoc), 3600, (err)=>{if(err) console.error(err)});
                    resolve();
                } else {
                    reject(err);
                }
        ...
        return new When.promise(function (resolve, reject) {
            userDB.userBasicColl.findOne({username: username}, function (err, result) {
                if(!err && result !== null) {
                    ...
                    if(memcached) memcached.set(MEMCACHED_USERID+result._id, JSON.stringify(result), 3600, ()=>{});
                    if(memcached) memcached.set(MEMCACHED_USERNAME+result.username, JSON.stringify(result), 3600, ()=>{});
    ...
    function createSession(userBasic){
        return new When.promise(function (resolve, reject) {
            ...
            loginDB.sessionColl.insertOne(session, function (err, result) {
                ...
                if(memcached) memcached.set(MEMCACHED_SESSION+sessionToken, JSON.stringify(session), 3600, ()=>{});
                resolve(session);
            });
        });
    }
    ...
    function getSession() {
        return new When.promise(function (resolve, reject) {
            loginDB.sessionColl.findOne({sessionToken: sessionToken}, function (err, result) {
                ...
                if (result !== null) {
                    if(memcached) memcached.set(MEMCACHED_SESSION+sessionToken, JSON.stringify(result), 3600, ()=>{});
                    return resolve(result);
                }
